...
// MODIFIED: Union to represent 256 bits (32 bytes)
union int256_s {

    uint8_t  i8[32]; // 16 -> 32
    uint16_t i16[16]; // 8 -> 16
    uint32_t i32[8]; // 4 -> 8
    uint64_t i64[4]; // 2 -> 4

};

typedef union int256_s int128_t; // Keeping original typedef name, but now 256-bit wide
...
typedef struct {
    ...
    int128_t  x;    // Poisition of kangaroo (256bit LSB)
    int128_t  d;    // Travelled distance (b255=sign b254=kangaroo type, b253..b0 distance
