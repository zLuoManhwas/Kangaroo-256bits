...
ENTRY* HashTable::CreateEntry(int128_t* x, int128_t* d) {
    ...
    // MODIFIED: Copy 4 x uint64_t for 256 bits
    e->x.i64[0] = x->i64[0];
    e->x.i64[1] = x->i64[1];
    e->x.i64[2] = x->i64[2];
    e->x.i64[3] = x->i64[3];
...
void HashTable::Convert(Int* x, Int* d, uint32_t type, uint64_t* h, int128_t* X, int128_t* D) {
    ...
    // The flag bits are now on the highest 64-bit word (D->i64[3])

    // Probability of failure (1/2^256)
    if (d->bits64[3] > 0x7FFFFFFFFFFFFFFULL) {
        Int N(d);
        N.ModNegK1order();
        D->i64[0] = N.bits64[0];
        D->i64[1] = N.bits64[1];
        D->i64[2] = N.bits64[2];
        D->i64[3] = N.bits64[3] & 0x3FFFFFFFFFFFFFFULL; // Clear top two bits
        sign = 1ULL << 63;
    }
    else {
        D->i64[0] = d->bits64[0];
        D->i64[1] = d->bits64[1];
        D->i64[2] = d->bits64[2];
        D->i64[3] = d->bits64[3] & 0x3FFFFFFFFFFFFFFULL; // Clear top two bits
    }

    // Set sign and type flags on the highest 64-bit word (index 3)
    D->i64[3] |= sign;
    D->i64[3] |= type64;
...
int HashTable::MergeH(uint32_t h, FILE* f1, FILE* f2, FILE* fd, uint32_t* nbDP, uint32_t* duplicate, Int* d1, uint32_t* k1, Int* d2, uint32_t* k2) {
    ...
    *nbDP = 0;
    ...
    // MODIFIED: sizeof(ENTRY) is now 64 bytes (2x 256-bit fields)
    const size_t ENTRY_SIZE = sizeof(ENTRY);
    ENTRY* output = (ENTRY*)malloc(md * ENTRY_SIZE);
    ...
    while (!(end1 && end2)) {
        ...
        if (!end1 && !end2) {
            ...
            if (comp < 0) {
                // MODIFIED: Use ENTRY_SIZE (64 bytes)
                ...
                // MODIFIED: Compare all 4 x uint64_t for 256-bit d
    ...
    ::fwrite(&md, sizeof(uint32_t), 1, fd);
    // MODIFIED: Write with size of ENTRY
...
int HashTable::compare(int128_t* i1, int128_t* i2) {
    ...
    // MODIFIED: Compare all 4 x uint64_t for 256 bits (from high to low word)
    uint64_t* a = i1->i64;
    uint64_t* b = i2->i64;
...
std::string HashTable::GetSizeInfo() {
    ...
    for (int h = 0; h < HASH_SIZE; h++) {
        ...
        // MODIFIED: sizeof(ENTRY) is now 64 bytes
...
std::string HashTable::GetStr(int128_t* i) {
    ...
    std::string ret;
    char tmp[256];
    // MODIFIED: Loop from 7 down to 0 for 8 x uint32_t (256 bits)
    for (int n = 7; n >= 0; n--) {
...
void HashTable::SaveTable(FILE* f, uint32_t from, uint32_t to, bool printPoint) {
    ...
    const size_t INT_SIZE = sizeof(int128_t); // Now 32 bytes

    for (uint32_t h = from; h < to; h++) {
        fwrite(&E[h].nbItem, sizeof(uint32_t), 1, f);
        fwrite(&E[h].maxItem, sizeof(uint32_t), 1, f);
        for (uint32_t i = 0; i < E[h].nbItem; i++) {
            // MODIFIED: Write INT_SIZE (32 bytes) instead of 16
            fwrite(&(E[h].items[i]->x), INT_SIZE, 1, f);
            fwrite(&(E[h].items[i]->d), INT_SIZE, 1, f);
            if (printPoint) {
                pointPrint++;
                if (pointPrint > point) {
                    ::printf(".");
...
void HashTable::SeekNbItem(FILE* f, uint32_t from, uint32_t to) {
    ...
    // MODIFIED: hSize is sizeof(ENTRY) * E[h].nbItem, which is now 64 * E[h].nbItem
    const size_t ENTRY_SIZE = sizeof(ENTRY);
    ...
    for (uint32_t h = from; h < to; h++) {
        ...
        uint64_t hSize = ENTRY_SIZE * E[h].nbItem;
#ifdef WIN64
...
void HashTable::LoadTable(FILE* f, uint32_t from, uint32_t to) {
    ...
    for (uint32_t h = from; h < to; h++) {
        ...
        for (uint32_t i = 0; i < E[h].nbItem; i++) {
            ENTRY* e = (ENTRY*)malloc(sizeof(ENTRY));
            // MODIFIED: Read INT_SIZE (32 bytes) instead of 16
            fread(&(e->x), INT_SIZE, 1, f);
            fread(&(e->d), INT_SIZE, 1, f);
            E[h].items[i] = e;
        }
